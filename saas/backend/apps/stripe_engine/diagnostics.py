...
def run_diagnostics(project: Project, project_root: Path) -> DiagnosticReport:
    ...
    if ".env" not in gitignore:
        _push(
            ...
            StripeIssue(
                ...
                auto_fixable=True,
                fix_action="fix-gitignore",
            ),
        )

    deps = scan.get("dependencies") or []
    dev_deps = scan.get("dev_dependencies") or []
    if scan.get("existing_stripe_code") and "stripe" not in deps and "stripe" not in dev_deps:
        if project.framework not in ("django", "flask", "rails", "laravel"):
            _push(
                issues,
                StripeIssue(
    ...
    if not _file_exists(project_root / "stripe.config.json"):
        _push(
            ...
            StripeIssue(
                ...
                auto_fixable=True,
                fix_action="create-stripe-config",
            ),
        )
    ...
    if missing and scan.get("suggested_features"):
    ...
    env_files = scan.get("env_files") or [".env.local"]
    env_file = env_files[0] if env_files else ".env.local"
    if _file_exists(project_root / env_file) and any(k.startswith("STRIPE_") for k in vault_keys):
        if "STRIPE_SECRET_KEY" in vault_keys and not _env_has_key(project_root, env_file, "STRIPE_SECRET_KEY"):
            _push(
                issues,
                StripeIssue(
                    id="env-out-of-sync",
    ...
    if not issues:
        ...
        summary = f"Found {len(issues)} issue(s): {errors} error(s), {warnings} warning(s). {fixable} auto-fixable."
