...
def derive_project_key(salt: bytes) -> bytes:
...
def encrypt_secret(plaintext: str, project_salt: bytes) -> EncryptedPayload:
    key = derive_project_key(project_salt)
    iv = os.urandom(12)
    aesgcm = AESGCM(key)
...
def decrypt_secret(payload: EncryptedPayload, project_salt: bytes) -> str:
    key = derive_project_key(project_salt)
